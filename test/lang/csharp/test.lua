...
do
  local inputs = {
    ...
    { 'struct declaration from initializer type',        { 27, 20 }, { 3, 14 } },
    { 'class declaration from initializer type',         { 28, 20 }, { 7, 13 } },
    { 'record declaration from initializer type',        { 29, 20 }, { 15, 14 } },
    { 'record struct declaration from initializer type', { 30, 21 }, { 17, 21 } },
    { 'generic class declaration from initializer type', { 31, 20 }, { 19, 13 } },
  ...
  for _, v in ipairs(inputs) do
    test('Go to ' .. v[1] .. ' declared outside accessing class', function ()
      ...
      if assert.not_nil(results, 'results') then
        assert.count(results, 2, 'results')
        assert.contains(results, { filename = 'test/lang/csharp/files/properties_other_class_from.cs', row = v[3][1], col = v[3][2] })
        assert.contains(results, { filename = 'test/lang/csharp/files/properties_other_class_to.cs', row = v[3][1], col = v[3][2] })
      end
    end)
  end
end
...
do
  local inputs = {
    { 'property passed as parameter',         { 16, 17 }, { 7, 16 } },
    { 'property asigned to variable',         { 17, 15 }, { 7, 16 } },
    { 'method call asigned to variable',      { 18, 15 }, { 9, 13 } },
    { 'property acccessed in a statement',    { 19, 7 },  { 7, 16 } },
    { 'method call acccessed in a statement', { 20, 7 },  { 9, 13 } },
  }
  for _, v in ipairs(inputs) do
    test('Go to ' .. v[1] .. ' declared outside accessing class in imported namespace', function ()
      ...
      if assert.not_nil(results, 'results') then
        assert.count(results, 2, 'results')
        assert.contains(results, { filename = 'test/lang/csharp/files/properties_ns_other_class_from.cs', row = v[3][1], col = v[3][2] })
        assert.contains(results, { filename = 'test/lang/csharp/files/properties_ns_other_class_to.cs', row = v[3][1], col = v[3][2] })
      end
    end)
  end
end
